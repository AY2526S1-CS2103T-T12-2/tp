...
public class EditCommandParser implements Parser<EditCommand> {
    ...
    public EditCommand parse(String args) throws ParseException {
        ...
        if (argMultimap.getValue(PREFIX_TUTORIAL_ID).isPresent()) {
            editTutorialDescriptor.setName(ParserUtil.parseTutorialId(argMultimap.getValue(PREFIX_TUTORIAL_ID).get()));
        }
    ...
    private Optional<Set<Student>> parseStudentsForEdit(Collection<String> students) throws ParseException {
        ...
        Collection<String> studentSet = students.size() == 1 && students.contains("")
                ? Collections.emptySet() : students;
