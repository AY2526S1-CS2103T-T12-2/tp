    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Tutorial tutorial = model.getTAbs().getTutorialList().stream()
                ...
                .orElseThrow(() -> new CommandException(
                        String.format("A tutorial with the TUTORIAL_ID %s does not exist", tutorialId.tutorialId)
                ));
        ...
        if (studentList.size() == 0) {
            throw new CommandException(
                    String.format("The tutorial %s has no students enrolled.", tutorialId.tutorialId)
            );
        }
